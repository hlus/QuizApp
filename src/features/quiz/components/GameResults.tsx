...
class GameResults extends Component<Props> {
    public get score(){ return this.props.questions.filter(q => q.result).length;}
    ...
    private handlePlayAgain = () => {
        this.props.navigation.reset({index: 0, routes: [{ name: RootScreens.Start }]});
    }
    ...
    public render() {
        return (
            <SafeAreaView style={styles.safeArea}>
                ...
                <Text style={styles.scoreText}>{this.score} / {this.props.questions.length}</Text>
                <FlatList
